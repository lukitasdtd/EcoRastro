...
const CalendarGrid = ({ currentDate, selectedDate, cropsForMonth, onDayClick, isLoading }: { 
    ...
    const calendarDays = useMemo(() => {
        ...
        days.push(...daysInMonthArray.map(day => {
            ...
            const dayContent = (
                <div onClick={() => onDayClick(day, cropForDay)} className={`text-center bg-card aspect-square flex flex-col justify-center items-center rounded-lg cursor-pointer transition-all duration-200 group relative`}>
                    ...
                    <span className={`font-semibold z-10 ${isToday(day) ? 'font-extrabold text-primary' : 'text-foreground'} group-hover:text-black`}>{format(day, 'd')}</span>
                    <div className="flex gap-1.5 mt-1.5 z-10 items-center justify-center h-4">
                        {cropForDay ? <CropIcon cropId={cropForDay.id} /> : (day.getDate() % 7 === 0 && <Moon className={`h-3 w-3 text-slate-500/70`} />)}
                    </div>
                </div>
            );
