...
export async function GET(
  ...
  const { rut } = params;
  const backendUrl = `http://localhost:5000/api/users/${rut}/gardens`;

  console.log(`API_PROXY_GARDENS: Reenviando petición (SIN CACHÉ) para el RUT: ${rut} a ${backendUrl}`);
  ...
  try {
    ...
    if (!apiResponse.ok) {
      console.error(`API_PROXY_GARDENS: Error desde el backend - Status: ${apiResponse.status}`);
      const errorBody = await apiResponse.text();
      console.error(`API_PROXY_GARDENS: Cuerpo del error - ${errorBody}`);
      return new NextResponse(errorBody, { status: apiResponse.status, statusText: apiResponse.statusText });
    }

    const data = await apiResponse.json();
    console.log(`API_PROXY_GARDENS: Petición exitosa, datos recibidos del backend.`);
    return NextResponse.json(data);

  } catch (error) {
    console.error('--- ERROR EN EL PROXY DE HUERTAS ---', error);
